...
@if($request['brand_id'] == 'all_brands')
    <h3 style="margin-top:40px;">All Brands Sale Report</h3>
@else
    @php
        $brand = App\Models\Product\Brand\ProductBrand::find($request['brand_id']);
    @endphp
    <h3 style="margin-top:40px;">{{ $brand ? $brand->name : 'Unknown Brand' }} Sale Report</h3>
@endif
                    ...
                    @isset($sale_data)
                        ...
                        @foreach($sale_data as $sale)
                            <tr>
                                ...
                                <td style="border:1px solid black;">{{ $sale->brand_name }}</td>
