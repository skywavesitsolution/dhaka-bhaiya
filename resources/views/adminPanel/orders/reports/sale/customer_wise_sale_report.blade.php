...
@if($request['customer_id'] == 'all_customers')
    <h3 style="margin-top:40px;">All Customer Sale Report</h3>
@else
    @php
        $party = App\Models\Party::find($request['customer_id']);
    @endphp
    <h3 style="margin-top:40px;">{{ $party ? $party->name : 'Unknown Party' }} Sale Report</h3>
@endif
                    ...
                    @isset($sale_data)
                        ...
                        @foreach($sale_data as $sale)
                            <tr>
                                ...
                                <td style="border:1px solid black;">{{ $sale->customer_name }}</td>
