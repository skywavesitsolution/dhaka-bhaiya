...
@section('content')
    ...
    @if ($request['category_id'] == 'all_category')
        <h3 style="margin-top:40px;">All Category Purchase Report</h3>
    @else
        @php
        $category = App\Models\Product\Category\ProductCategory::find($request['category_id']);
        @endphp
        <h3 style="margin-top:40px;">{{ $category ? $category->name : 'Unknown Category' }} Purchase Report</h3>
    @endif
    ...
    <div class="row pl-5 pr-5">
        ...
        $category = App\Models\Product\Category\ProductCategory::find($request['category_id']);
                @endphp
                <h6>Report Type: {{ $category ? $category->name : 'Unknown Category' }}</h6>
            @endif
        </div>
        <div class="col-md-9">
                        ...
                        @isset($purchase_data)
                            ...
                            @foreach ($purchase_data as $purchase)
                                <tr>
                                    ...
                                    <td style="border:1px solid black;">{{ $purchase->category_name}}</td>
