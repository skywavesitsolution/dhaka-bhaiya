...
@section('content')
    ...
    @if ($request['location_id'] == 'all_location')
        <h3 style="margin-top:40px;">All Location Purchase Report</h3>
    @else
        @php
        $location = App\Models\Product\Location\ProductLocation::find($request['location_id']);
        @endphp
        <h3 style="margin-top:40px;">{{ $location ? $location->name : 'Unknown location' }} Purchase Report</h3>
    @endif
    ...
    <div class="row pl-5 pr-5">
        ...
        <div class="col-md-3">
            @if ($request['location_id'] == 'all_location')
                <h6>Report Type: All Location</h6>
            @else
                @php
            $location = App\Models\Product\Location\ProductLocation::find($request['location_id']);
                @endphp
                <h6>Report Type: {{ $location ? $location->name : 'Unknown Location' }}</h6>
            @endif
        </div>
        <div class="col-md-9">
                        ...
                        @isset($purchase_data)
                            ...
                            @foreach ($purchase_data as $purchase)
                                <tr>
                                    ...
                                    <td style="border:1px solid black;">{{ $purchase->location_name}}</td>
