...
@section('content')
    ...
    @if ($request['supplier_id'] == 'all_supplier')
        <h3 style="margin-top:40px;">All Supplier Purchase Report</h3>
    @else
        @php
         $party = App\Models\Party::find($request['supplier_id']); 
        @endphp
        <h3 style="margin-top:40px;">{{ $party ? $party->name : 'Unknown Supplier' }} Purchase Report</h3>
    @endif
    ...
    <div class="row pl-5 pr-5">
        ...
        <div class="col-md-3">
            ...
            @else
                ...
                $party = App\Models\Party::find($request['supplier_id']); 
                @endphp
                <h6>Report Type: {{ $party ? $party->name : 'Unknown supplier' }}</h6>
            @endif
        </div>
        <div class="col-md-9">
                        ...
                        @isset($purchase_data)
                            ...
                            @foreach ($purchase_data as $purchase)
                                <tr>
                                    ...
                                    <td style="border:1px solid black;">{{ $purchase->supplier_name}}</td>
