...
@if($request['product_id'] == 'all_products')
    <h3 style="margin-top:40px;">All Products Stock Report</h3>
 @else
     @php
         $product = App\Models\Product\Product::find($request['product_id']); 
     @endphp
     <h3 style="margin-top:40px;">{{ $product ? $product->product_name : 'Unknown category' }} Stock Report</h3>
 @endif
    
@endisset
 @isset($request['category_id'])
         @if($request['category_id'] == 'all_category')
    <h3 style="margin-top:40px;">All Categories</h3>
            @else
            @php
                $category = App\Models\Product\Category\ProductCategory::find($request['category_id']); 
            @endphp
            <h3 style="margin-top:40px;"> {{ $category ? $category->name : 'Unknown Product' }}</h3>
        @endif
...
<div class="row pl-5 pr-5">
    ...
    <div class="col-md-3">
        ...
        @else
         ...
         $product = App\Models\Product\Product::find($request['product_id']); 
            @endphp
            <h6>Report Type: {{ $product ? $product->product_name : 'Unknown Product' }} Stock</h6>
        @endif
    ...
    <div class="col-md-9">
        @isset($request['category_id'])
         @if($request['category_id'] == 'all_category')
            <h6>Category Name: All Categories</h6>
            @else
            @php
                $category = App\Models\Product\Category\ProductCategory::find($request['category_id']); 
            @endphp
            <h6>Category Name: {{ $category ? $category->name : 'Unknown Product' }}</h6>
        @endif
    ...
    <div class="col-md-3">
       @isset($request['product_id'])
        @if($request['product_id'] == 'all_products')
            <h6>Product Name: All Products</h6>
            @else
            @php
         $product = App\Models\Product\Product::find($request['product_id']); 
            @endphp
            <h6>Product ID: {{ $product ? $product->product_name : 'Unknown Product' }} Stock</h6>
        @endif
                     ...
                     @php
                        ...
                        @foreach($stock_data as $stock)
                            <tr>
                                ...
                                <td style="border:1px solid black;">{{ $stock->category_name }}</td>
