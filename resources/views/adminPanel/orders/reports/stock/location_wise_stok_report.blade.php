...
@if($request['location_id'] == 'all_locations')
    <h3 style="margin-top:40px;">All Locations Stock Report</h3>
 @else
     @php
        $location = App\Models\Product\Location\ProductLocation::find($request['location_id']); 
     @endphp
     <h3 style="margin-top:40px;">{{ $location ? $location->name : 'Unknown Location' }} Stock Report</h3>
 @endif
...
<div class="row pl-5 pr-5">
    ...
    <div class="col-md-3">
        @isset($request['location_id'])
        @if($request['location_id'] == 'all_locations')
            <h6>Report Type: All Locations</h6>
        @else
            @php
        $location = App\Models\Product\Location\ProductLocation::find($request['location_id']); 
            @endphp
            <h6>Report Type: {{ $location ? $location->name : 'Unknown Location' }} Stock</h6>
        @endif
                     ...
                     @php
                        ...
                        @foreach($stock_data as $stock)
                            <tr>
                                ...
                                <td style="border:1px solid black;">{{ $stock->location_name }}</td>
