                            ...
                            @foreach ($sortedArray as $ledgerItem)
                                ...
                                if (isset($ledgerItem->purchase_id)) {
                                    ...
                                    if ($party->type == 'Supplier') {
                                                    ?>
                                                    <h6>Purchase ID: {{ $order->purchase_id }} | Supplier: {{ $order->party_id }} </h6>
                                    ...
                                    if ($party->type == 'Customer') {
                                                        ?>
                                                    <h6>Supplier: {{ $order->party_id }} | Order ID: {{ $order->sale_id }} | Product:
                                                        {{ $productType }}| Qty:
                                                        {{ number_format($order->purchase_qty) }} | Sale Price:
                                                        {{ number_format($order->sale_rate) }}
                                                    </h6>
                                ...
                                if ($party->type == 'Customer') {
                                    ...
                                    // if (isset($ledgerItem->sale_id )) {
                                    //     if ($ledgerItem->price >= 0) {
                                    //         echo number_format($ledgerItem->price);
                                    //     } else {
                                    //         echo "[ " . number_format($ledgerItem->price) . " ]";
                                    //     }
                                    //     $total_debit += $ledgerItem->price;
                                    // }
                                } else {
                                    if (isset($ledgerItem->payment_id)) {
                                                        ?>
                                                ...
                                                <td style="border:1px solid black;">
                                                    <?php
                                if ($party->type == 'Customer') {
                                    // Sale amount for customer
                                    if (isset($ledgerItem->price)) {
                                        if ($ledgerItem->price >= 0) {
                                            echo number_format($ledgerItem->price);
                                        } else {
                                            echo '[ ' . number_format($ledgerItem->price) . ' ]';
                                        }
                                        $total_credit += $ledgerItem->price; // Add to total debit for customers
                                    }
                                    ...
                                    if (isset($ledgerItem->payment_id)) {
                                        ...
                                        $total_credit += $ledgerItem->payment; // Add to total debit
                                    }
                                } else {
                                    // For other party types like Supplier or Driver, similar logic can be applied for payments and purchases
